...
import {
  ...
  advanceSubmodulesToOriginMain,
  submoduleDirsFor,
} from "../skills/submoduleGit.js";
import { runRescriptBuild } from "../skills/buildCheck.js";
import { formatPrBody, publishPullRequest } from "../skills/githubPr.js";
...
patchesRouter.post("/gaps/:id/patch", async (req, res) => {
  ...
  try {
    ...
    try {
      const body = formatPrBody({ gapId, canonicalName: gap.canonical_name, category: gap.category, rationale: gap.rationale, summaryJson: summary, filesTouched: fileCount, buildLog: build.log });
...
Do NOT write any code in the target repo. Your cwd is ${sourceDir} — only read there.`;
      ...
      writeLine({
        type: "text",
        text: "Scanning the complete branch for secrets, then publishing to the canonical juspay repository…",
      });
...
patchesRouter.post("/patches/:id/force-pr", async (req, res) => {
  ...
  try {
    await withRepoLock(targetRepo, async () => {
      ...
      // Which submodule pointers moved between main and this branch. Asking the
      // parent is immune to a submodule sitting in detached HEAD. publishPullRequest
      // refuses to publish these (they need their own upstream PRs first), so the
      // user gets that explicit message instead of a half-published feature.
      const submodulesWithCommits: string[] = [];
      for (const subDir of submoduleDirsFor(targetRepo)) {
        if (!fs.existsSync(path.join(targetDir, subDir, ".git"))) continue;
        const pointerDiff = await git.raw(["diff", "main", "--", subDir]).catch(() => "");
      ...
      try {
        const body = formatPrBody({
          ...
          buildLog: null,
        }) + `\n\n> ⚠️ This PR was forced through despite the mobile native-wrapper gate. Native wrappers may be incomplete — verify manually before merge.`;
        const created = await publishPullRequest({
          repoDir: targetDir,
          repoKey: targetRepo,
          branch: branchName,
          title: `feat: add ${gap.canonical_name}`,
          body,
          submodulesChanged: submodulesWithCommits,
        });
        prUrl = created.prUrl;
        prNumber = created.prNumber;
      } catch (err) {
