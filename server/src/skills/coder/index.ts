 ...
 *
 * Unlike the integration skill, this doesn't require SDK documentation,
 ...
 *     workspace (single coder for interface consistency), same as integration.
...
function buildSingleRepoCoderPrompt(task: string, repoKey: RepoKey, additionalContext?: string): string {
  const repoName = REPOS[repoKey].name;
  const descriptions: Record<RepoKey, string> = {
...
- \`hyperswitch-web/\` — the web SDK repo (ReScript + React)`,
  ...
  return buildCoderPrompt(task, descriptions[repoKey] || `Working in ${repoName}.`, additionalContext);
}
