...
export function PropsForm({ onResult, onError }: SkillFormProps) {
  ...
  const onSubmit = async () => {
    ...
    setGenerating(true);
    try {
      const result = await api.generatePropsSkill(spec);
      onResult(result);
    } catch (e) {
  ...
  return (
    <div className="rounded-xl border border-slate-800 bg-slate-900/50 p-6">
      <h2 className="text-lg font-semibold text-slate-100 mb-1">Add New Prop</h2>
      <p className="text-sm text-slate-500 mb-6">
        Define a configuration prop and the AI will implement it across both SDKs following each repo's existing patterns.
      </p>

      <div className="grid grid-cols-2 gap-4 mb-4">
        <div>
          <label className="block text-xs text-slate-400 mb-1">Prop Name</label>
          <input
            type="text"
            value={propName}
        ...
        <div>
          <label className="block text-xs text-slate-400 mb-1">
            Parent Config <span className="text-slate-600">(optional)</span>
          </label>
          <input
            type="text"
            value={parentConfig}
      ...
      <div className="mb-4">
        <label className="block text-xs text-slate-400 mb-1">Behavior Description</label>
        <textarea
          value={behavior}
          onChange={(e) => setBehavior(e.target.value)}
          rows={3}
          placeholder="When true, always show billing/dynamic fields regardless of whether they are pre-filled."
